param(
    [string]$SuggestionsCsv = "C:\test\Shrimp\Assets\ThirdPersonController\Reports\boss_attack_tuning_round5_auto_suggestions.csv",
    [string]$FillCsv = "C:\test\Shrimp\Assets\ThirdPersonController\Reports\boss_attack_tuning_round4_fill.csv",
    [string]$ReportCsv = "C:\test\Shrimp\Assets\ThirdPersonController\Reports\boss_attack_tuning_round5_apply_report.csv",
    [string]$BackupCsv = ""
)
...
foreach ($row in $fillRows) {
    ...
    $damage = To-Text (Parse-Double $s.round5_damage (Parse-Double $row.damage 0))
    $cooldown = To-Text (Parse-Double $s.round5_cooldown (Parse-Double $row.cooldown 0))
    $selection = To-Text (Parse-Double $s.round5_selection_weight (Parse-Double $row.selection_weight 1))
    $windup = To-Text (Parse-Double $s.round5_windup_time (Parse-Double $row.windup_time 0))
    $active = To-Text (Parse-Double $s.round5_active_time (Parse-Double $row.active_time 0))
    $recovery = To-Text (Parse-Double $s.round5_recovery_time (Parse-Double $row.recovery_time 0))
    $range = To-Text (Parse-Double $s.round5_range (Parse-Double $row.range 0))
    $knockback = To-Text (Parse-Double $s.round5_knockback_force (Parse-Double $row.knockback_force 0))
    $aoeRadius = To-Text (Parse-Double $s.round5_aoe_radius (Parse-Double $row.aoe_radius 0))
