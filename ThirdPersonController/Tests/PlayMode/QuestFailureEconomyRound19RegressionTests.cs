...
{
    public class QuestFailureEconomyRound19RegressionTests
    {
        ...
        [Test]
        public void QuestSystem_Round19_FailureTypeByChapterMatrix_TriggersNativeFailPaths_AndKeepsRecoveryWindowPlayable()
        {
            EconomyConfig config = BuildP1SimulationConfig();
            EconomyService.Configure(config);

            QuestSystem questSystem = CreateQuestRuntime(
                "QuestFailureEconomy_Round19_Matrix",
            ...
            List<FailureMatrixScenario> scenarios = BuildRound19MatrixScenarios();
            Assert.AreEqual(15, scenarios.Count, "Round19 matrix should cover 3 chapters x 5 failure types.");
            ...
            for (int i = 0; i < scenarios.Count; i++)
            {
                ...
                QuestData failureQuest = NewQuestData(
                    questId: $"r19_fail_{scenario.chapterId}_{scenario.failureRuleType}_{i}",
                ...
                QuestData recoveryQuest = NewQuestData(
                    questId: $"r19_recover_{scenario.chapterId}_{scenario.failureRuleType}_{i}",
                    questType: scenario.recoveryQuestType,
            ...
            {
                Assert.GreaterOrEqual(pair.Value, 3, $"Failure type coverage should include all configured chapters: {pair.Key}");
            }

            Assert.AreEqual(expectedExpTotal, experienceSystem.currentExp, "Round19 matrix EXP should equal successful recovery payouts only.");
            Assert.AreEqual(expectedCreditsTotal, wallet.Credits, "Round19 matrix credits should equal successful recovery payouts only.");
        ...
        [Test]
        public void EconomyService_Round19_FailureTypeByChapterRecoveryDebtMatrix_RemainsRecoverable()
        {
            EconomyConfig config = BuildP1SimulationConfig();
            EconomyService.Configure(config);
        ...
        private static List<FailureMatrixScenario> BuildRound19MatrixScenarios()
        {
            ...
            {
                int levelDifficulty = chapter <= 3 ? 3 : (chapter == 4 ? 4 : 5);
                string strongholdId = chapter == 3 ? "SH_MID" : (chapter == 4 ? "SH_LATE" : "SH_END");
                int chapterOffset = chapter == 3 ? 0 : (chapter == 4 ? 24 : 48);
